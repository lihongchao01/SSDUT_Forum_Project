...
MagicZoom.prototype.initZoom = function() {
    ...
    this.bigImageCont.style.left = this.smallImage.width + 65 + 'px';
    this.bigImageCont.style.top = '0px';
    switch (this.settings['position']) {
    case 'left':
        this.bigImageCont.style.left = '-' + (100 + parseInt(this.bigImageCont.style.width)) + 'px';
        break;
    case 'bottom':
...
function MagicZoom_findZooms() {
    ...
    for (var i = 0; i < aels.length; i++) {
        if (/MagicZoom/.test(aels[i].className)) {
            ...
            matches = re.exec(aels[i].rel);
            bigCont.style.width = '380px';
            if (matches) {
                bigCont.style.width = matches[3]
            }
            re = new RegExp(/zoom\-height(\s+)?:(\s+)?(\w+)/i);
            matches = re.exec(aels[i].rel);
            bigCont.style.height = '380px';
            if (matches) {
